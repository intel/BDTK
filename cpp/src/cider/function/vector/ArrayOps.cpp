...
extern "C" RUNTIME_EXPORT int32_t array_size(int8_t* chunk_iter_,
...
extern "C" RUNTIME_EXPORT int32_t array_size_nullable(int8_t* chunk_iter_,
                                                      const uint64_t row_pos,
                                                      const uint32_t elem_log_sz,
...
extern "C" RUNTIME_EXPORT bool array_is_null(int8_t* chunk_iter_,
                                             const uint64_t row_pos) {
  ChunkIter* chunk_iter = reinterpret_cast<ChunkIter*>(chunk_iter_);
  ArrayDatum ad;
  bool is_end;
...
extern "C" RUNTIME_EXPORT bool point_coord_array_is_null(int8_t* chunk_iter_,
                                                         const uint64_t row_pos) {
  ChunkIter* chunk_iter = reinterpret_cast<ChunkIter*>(chunk_iter_);
  ArrayDatum ad;
  bool is_end;
...
#define ARRAY_AT(type)                                                        \
  extern "C" RUNTIME_EXPORT type array_at_##type(                             \
...
#define VARLEN_ARRAY_AT(type)                                                 \
  extern "C" RUNTIME_EXPORT type varlen_array_at_##type(                      \
      int8_t* chunk_iter_, const uint64_t row_pos, const uint32_t elem_idx) { \
    ChunkIter* chunk_iter = reinterpret_cast<ChunkIter*>(chunk_iter_);        \
    ArrayDatum ad;                                                            \
    bool is_end;                                                              \
...
#define VARLEN_NOTNULL_ARRAY_AT(type)                                         \
  extern "C" RUNTIME_EXPORT type varlen_notnull_array_at_##type(              \
      int8_t* chunk_iter_, const uint64_t row_pos, const uint32_t elem_idx) { \
    ChunkIter* chunk_iter = reinterpret_cast<ChunkIter*>(chunk_iter_);        \
    ArrayDatum ad;                                                            \
    bool is_end;                                                              \
...
#define ARRAY_ANY(type, needle_type, oper_name, oper)                            \
  extern "C" RUNTIME_EXPORT bool array_any_##oper_name##_##type##_##needle_type( \
      int8_t* chunk_iter_,                                                       \
      const uint64_t row_pos,                                                    \
...
#define ARRAY_ALL(type, needle_type, oper_name, oper)                            \
  extern "C" RUNTIME_EXPORT bool array_all_##oper_name##_##type##_##needle_type( \
      int8_t* chunk_iter_,                                                       \
      const uint64_t row_pos,                                                    \
      const needle_type needle,                                                  \
      const type null_val) {                                                     \
...
#define ARRAY_AT_CHECKED(type)                                                     \
  extern "C" RUNTIME_EXPORT type array_at_##type##_checked(int8_t* chunk_iter_,    \
                                                           const uint64_t row_pos, \
...
extern "C" RUNTIME_EXPORT int8_t* allocate_varlen_buffer(int64_t element_count,
...
extern "C" RUNTIME_EXPORT ALWAYS_INLINE int8_t* fast_fixlen_array_buff(
    int8_t* chunk_iter_,
    const uint64_t row_pos) {
  ChunkIter* it = reinterpret_cast<ChunkIter*>(chunk_iter_);
...
extern "C" RUNTIME_EXPORT int8_t* array_buff(int8_t* chunk_iter_,
                                             const uint64_t row_pos) {
  ChunkIter* chunk_iter = reinterpret_cast<ChunkIter*>(chunk_iter_);
  ArrayDatum ad;
  bool is_end;

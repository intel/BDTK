...
// But omnisci's codes will be removed in future and most functions in that dir will not
// be used. So redefine murmurhash function in this file
...
FORCE_INLINE uint32_t MurmurHash3_32(const int8_t* key,
                                     int len,
...
struct MurmurHash {
  uint32_t operator()(int32_t rawHash) { return murmurhash32(rawHash); }
  uint64_t operator()(int64_t rawHash) { return murmurhash64(rawHash); }
};

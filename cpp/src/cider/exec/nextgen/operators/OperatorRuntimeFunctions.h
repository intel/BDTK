...
RUNTIME_FUNC ALLOW_INLINE void nextgen_cider_agg_sum(ST& agg_val, const TT& val) {
  agg_val += val;
}
...
template <typename T>
RUNTIME_FUNC ALLOW_INLINE void nextgen_cider_agg_min(T& agg_val, const T& val) {
  agg_val = std::min(agg_val, val);
}
DEF_NEXTGEN_CIDER_SIMPLE_AGG_FUNCS(min, nextgen_cider_agg_min)

template <typename T>
RUNTIME_FUNC ALLOW_INLINE void nextgen_cider_agg_max(T& agg_val, const T& val) {
...
extern "C" RUNTIME_FUNC ALLOW_INLINE void nextgen_cider_agg_count(int64_t* agg_val_addr) {
  ++(*agg_val_addr);
}
...
extern "C" RUNTIME_FUNC ALLOW_INLINE size_t get_lowest_set_bit(size_t data) {
  return CiderBitUtils::countTailZero(data);
}

extern "C" RUNTIME_FUNC ALLOW_INLINE size_t reset_tail_set_bit(size_t data) {

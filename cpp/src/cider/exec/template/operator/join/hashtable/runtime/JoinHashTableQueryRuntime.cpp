...
bool compare_to_key(const int8_t* entry, const int8_t* key, const size_t key_bytes) {
...
template <class T>
int64_t get_matching_slot(const int8_t* hash_buff,
                          const uint32_t h,
                          const int8_t* key,
...
template <class T>
FORCE_INLINE int64_t baseline_hash_join_idx_impl(const int8_t* hash_buff,
                                                 const int8_t* key,
                                                 const size_t key_bytes,
...
template <typename T>
FORCE_INLINE int64_t get_bucket_key_for_value_impl(const T value,
...
template <typename T>
FORCE_INLINE int64_t get_composite_key_index_impl(const T* key,
...
extern "C" RUNTIME_EXPORT NEVER_INLINE int32_t insert_sorted(int32_t* arr,
                                                             size_t elem_count,
                                                             int32_t elem) {
...
ALWAYS_INLINE BufferRange get_row_id_buffer_ptr(int64_t* hash_table_ptr,
                                                const int64_t* key,
...
/// we insert can be considered relatively small (N < 200)
///
/// RETURNS:

...
// It is found through the code that the method of inserting elements into the map has not
// been used at present, so there are no elements in the map structure here, and the
// method will only return false.
bool HashtableRecycler::isSafeToCacheHashtable(
  ...
  auto getNodeByTableId = [&table_id_to_node_map](const int table_id) -> const void* {
  ...
  if (table_id < 0) {
    ...
    // TODO: Comment processing is done here, which is reserved for subsequent expansion.

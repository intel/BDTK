...
#include "exec/template/StreamingTopN.h"
#include "exec/template/UsedColumnsVisitor.h"
...
int8_t pick_baseline_key_component_width(const ExpressionRange& range,
  ...
  switch (range.getType()) {
    case ExpressionRangeType::Integer:
      // When using default range likes min=0, max= -1, Cider picks integer width as 4
      // bytes even data type is Long. This will lead to wrong query result, fixing the
      // bug and enable some UTs.
      if (group_col_width == sizeof(int64_t) &&
          (range.hasNulls() || range.getIntMax() < range.getIntMin())) {
        return sizeof(int64_t);
      }
  ...
  for (const auto& col_expr : col_expr_list) {
    if (!col_expr) {
      ...
      if ((chosen_type.is_string() && chosen_type.get_compression() == kENCODING_NONE) ||
        ...
        // All VarChar type group key should be encoded.
        col_widths.push_back(sizeof(int64_t));
        ++col_expr_idx;
        continue;
      }
...
QueryMemoryDescriptor::QueryMemoryDescriptor(
    ...
    , cider_data_format_(use_cider_data_format)
...
size_t QueryMemoryDescriptor::getRowSize() const {
  ...
  // Null vector of group targets
  if (cider_data_format_) {
    total_bytes += ((getBufferColSlotCount() + 7) >> 3);
  }
...
size_t QueryMemoryDescriptor::getNullVectorOffsetOfGroupTargets() const {
  CHECK(cider_data_format_);
  return getColsSize();
}
...
size_t QueryMemoryDescriptor::getBufferSizeBytes(const RelAlgExecutionUnit& ra_exe_unit,
  ...
  return getBufferSizeBytes(entry_count_);
}

/**
 * Returns total amount of output buffer memory for each device (CPU)
 *
 * Columnar:
...
void QueryMemoryDescriptor::alignPaddedSlots() {
  col_slot_context_.alignPaddedSlots();
}
...
std::string QueryMemoryDescriptor::toString() const {
  ...
  str += "\tBlocks Share Memory: " + ::toString(true) + "\n";
  ...
  str += "\tUse Cider Data Format: " + ::toString(cider_data_format_) + "\n";
  return str;
}

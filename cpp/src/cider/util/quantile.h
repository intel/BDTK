...
struct Centroids {
  ...
  void appendAndSortCurrent(Centroids& buff);

  IndexType capacity() const { return sums_.capacity(); }
  ...
  IndexType currCount() const { return counts_[curr_idx_]; }

  RealType currMean() const { return mean(curr_idx_); }

  bool hasCurr() const { return curr_idx_ < size(); }

  bool hasNext() const { return next_idx_ < size(); }

  RealType mean(IndexType const i) const { return sums_[i] / counts_[i]; }

  // Return true if centroid was merged, false if not.
  bool mergeIfFits(Centroids& centroid, IndexType const max_count);

  void moveNextToCurrent();

  IndexType nextCount() const { return counts_[next_idx_]; }

  RealType nextSum() const { return sums_[next_idx_]; }

  // Assumes this->hasNext() and b.hasNext().
  // Order by (nextMean(), -nextCount()).
  bool operator<(Centroids const& b) const {
  ...
  void push_back(RealType const value, RealType const count) {
    sums_.push_back(value);
    counts_.push_back(count);
  }

  void resetIndices(bool const forward);

  size_t size() const { return sums_.size(); }

  IndexType totalWeight() const {
    return std::accumulate(counts_.begin(), counts_.end(), IndexType(0));
  }
...
class CentroidsMerger {
  ...
  void mergeMinMax();
  void setCurrCentroid();

 public:
  CentroidsMerger(Centroids<RealType, IndexType>* buf,
                  Centroids<RealType, IndexType>* centroids,
                  bool const forward);

  // nullptr if !hasNext()
  Centroids<RealType, IndexType>* getNextCentroid() const;

  bool hasNext() const { return buf_->hasNext() || centroids_->hasNext(); }

  // Merge as many centroids as possible for count <= max_count.
  void merge(IndexType const max_count);

  // Assume curr_centroid_ is fully merged.
  void next();

  IndexType prefixSum() const { return prefix_sum_; }

  IndexType totalWeight() const { return total_weight_; }
};
...
class TDigest {
  ...
  RealType max() const { return centroids_.max_; }
  RealType min() const { return centroids_.min_; }

  IndexType maxCardinality(IndexType const sum,
                           IndexType const total_weight,
                           RealType const c);

  // Require: centroids are sorted by (mean, -count)
  void mergeCentroids(Centroids<RealType, IndexType>&);

  RealType firstCentroid(RealType const x);
  RealType interiorCentroid(RealType const x,
                            IndexType const idx1,
                            IndexType const prefix_sum);
  RealType lastCentroid(RealType const x, IndexType const N);
  RealType oneCentroid(RealType const x);

  RealType slope(IndexType const idx1, IndexType const idx2);
 ...
 public:
  ...
  TDigest(Memory& mem) : centroids_(mem.sums().data(), mem.counts().data(), mem.size()) {
    centroids_.clear();
  }
  ...
  Centroids<RealType, IndexType>& centroids() { return centroids_; }
  ...
  void mergeSorted(RealType* sums, IndexType* counts, IndexType size);

  void mergeTDigest(TDigest& t_digest) {
    mergeBuffer();
  ...
  RealType quantile(IndexType* buf, RealType const q);

  // Uses buf_ as scratch space.
  RealType quantile(RealType const q) {
    assert(centroids_.size() <= buf_.capacity());
    return quantile(buf_.counts_.data(), q);
  }

  RealType quantile() { return q_ ? quantile(*q_) : centroids_.nan; }

  // Assumes mem is externally managed.
  void setBuffer(Memory& mem) {
  ...
  // Assumes mem is externally managed.
  void setCentroids(Memory& mem) {
    centroids_ = Centroids<RealType, IndexType>(
        mem.sums().data(), mem.counts().data(), mem.size());
    centroids_.clear();
  }

  void setCentroids(VectorView<RealType> const sums, VectorView<IndexType> const counts) {
    centroids_ = Centroids<RealType, IndexType>(sums, counts);
    centroids_.clear();
  }

  // Total number of data points in centroids_.
  IndexType totalWeight() const { return centroids_.totalWeight(); }
};
...
template <typename RealType, typename IndexType>
void Centroids<RealType, IndexType>::appendAndSortCurrent(Centroids& buff) {
  if (inc_ == -1 && curr_idx_ != 0) {
    ...
    std::reverse(sums_.begin(), sums_.end());
  ...
  std::copy(buff_sums.begin(), buff_sums.end(), sums_.begin() + curr_size);
  assert(total_size <= counts_.capacity());
  counts_.resize(total_size);
  std::copy(buff_counts.begin(), buff_counts.end(), counts_.begin() + curr_size);
  ...
  std::sort(begin, end, OrderByMeanAscCountDesc<RealType, IndexType>());
}

// Return true if centroid was merged, false if not.
template <typename RealType, typename IndexType>
bool Centroids<RealType, IndexType>::mergeIfFits(Centroids& centroid,
...
template <typename RealType, typename IndexType>
void Centroids<RealType, IndexType>::moveNextToCurrent() {
  curr_idx_ += inc_;
...
template <typename RealType, typename IndexType>
void Centroids<RealType, IndexType>::resetIndices(bool const forward) {
  if (forward) {
    inc_ = 1;
...
template <typename RealType, typename IndexType>
CentroidsMerger<RealType, IndexType>::CentroidsMerger(
...
template <typename RealType, typename IndexType>
Centroids<RealType, IndexType>* CentroidsMerger<RealType, IndexType>::getNextCentroid()
    const {
  if (buf_->hasNext()) {
...
class Skipped {
  ...
  static void shiftCentroids(Data& data) {
  ...
  template <typename T>
  static void shiftRange(T* const begin,
                         IndexType skipped,
 ...
 public:
  ...
  void skipFirst(Centroids<RealType, IndexType>* next_centroid) {
  ...
  void skipSubsequent(Centroids<RealType, IndexType>* next_centroid) {
    IndexType const idx = index(next_centroid);
...
// Merge as many centroids as possible for count <= max_count.
template <typename RealType, typename IndexType>
void CentroidsMerger<RealType, IndexType>::merge(IndexType const max_count) {
...
// centroids.
template <typename RealType, typename IndexType>
void CentroidsMerger<RealType, IndexType>::mergeMinMax() {
...
// Assume curr_centroid_ is fully merged.
template <typename RealType, typename IndexType>
void CentroidsMerger<RealType, IndexType>::next() {
  prefix_sum_ += curr_centroid_->currCount();
  setCurrCentroid();
}

template <typename RealType, typename IndexType>
void CentroidsMerger<RealType, IndexType>::setCurrCentroid() {
...
template <typename RealType, typename IndexType>
void TDigest<RealType, IndexType>::add(RealType value) {
  if (buf_.sums_.full()) {
    mergeBuffer();
  }
...
template <typename RealType, typename IndexType>
void TDigest<RealType, IndexType>::allocate() {
  if (buf_.capacity() == 0) {
...
template <typename RealType, typename IndexType>
IndexType TDigest<RealType, IndexType>::maxCardinality(IndexType const sum,
                                                       IndexType const total_weight,
                                                       RealType const c) {
...
template <typename RealType, typename IndexType>
void TDigest<RealType, IndexType>::mergeBuffer() {
  if (buf_.size()) {
    std::sort(buf_.sums_.begin(), buf_.sums_.end());
...
template <typename RealType, typename IndexType>
void TDigest<RealType, IndexType>::mergeSorted(RealType* sums,
                                               IndexType* counts,
                                               IndexType size) {
  if (size) {
    if (buf_.capacity() == 0) {
    ...
    std::fill(buf_.counts_.begin(), buf_.counts_.end(), IndexType(1));
    buf_.min_ = buf_.sums_.front();
    buf_.max_ = buf_.sums_.back();
    mergeCentroids(buf_);
  }
...
template <typename RealType, typename IndexType>
void TDigest<RealType, IndexType>::mergeCentroids(Centroids<RealType, IndexType>& buf) {
...
bool isSingleton(CountsIterator itr) {
  return *itr == 1;
}
}  // namespace
...
template <typename RealType, typename IndexType>
RealType TDigest<RealType, IndexType>::firstCentroid(RealType const x) {
  if (x < 1) {
    return min();
...
template <typename RealType, typename IndexType>
RealType TDigest<RealType, IndexType>::interiorCentroid(RealType const x,
                                                        IndexType const idx1,
                                                        IndexType const prefix_sum) {
...
template <typename RealType, typename IndexType>
RealType TDigest<RealType, IndexType>::lastCentroid(RealType const x, IndexType const N) {
  if (N - 1 < x) {
    return max();
  }
...
template <typename RealType, typename IndexType>
RealType TDigest<RealType, IndexType>::oneCentroid(RealType const x) {
...
template <typename RealType, typename IndexType>
RealType TDigest<RealType, IndexType>::quantile(IndexType* buf, RealType const q) {
  if (centroids_.size()) {
    ...
    auto const it1 = std::upper_bound(partial_sum.begin(), partial_sum.end(), x);
...
template <typename RealType, typename IndexType>
RealType TDigest<RealType, IndexType>::slope(IndexType idx1, IndexType idx2) {

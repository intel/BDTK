...
struct StringKey24 {
  ...
  bool operator==(const StringKey24 rhs) const {
    return a == rhs.a && b == rhs.b && c == rhs.c;
  }
};

inline uint8_t countl_zero(uint64_t a) {
  std::bitset<64> bs(a);
  uint8_t n = 0;
...
inline StringRef ALWAYS_INLINE toStringRef(const StringKey8& n) {
  assert(n != 0);
  // TODO(Deegue): Enable after C++20 is supportted(countr_zero).
  // #if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  //   return {reinterpret_cast<const char*>(&n), 8ul - (countr_zero(n)
  //   >> 3)};
  // #else
  return {reinterpret_cast<const char*>(&n), 8ul - (countl_zero(n) >> 3)};
  // #endif
}
inline StringRef ALWAYS_INLINE toStringRef(const StringKey16& n) {
  ...
  //   (countr_zero(n.items[1]) >> 3)};
  // #else
  return {reinterpret_cast<const char*>(&n), 16ul - (countl_zero(n.items[1]) >> 3)};
  // #endif
}
inline StringRef ALWAYS_INLINE toStringRef(const StringKey24& n) {
  ...
  // TODO(Deegue): Enable after C++20 is supportted(countr_zero).
  // #if __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  //   return {reinterpret_cast<const char*>(&n), 24ul -
  //   (countr_zero(n.c) >> 3)};
  // #else
  return {reinterpret_cast<const char*>(&n), 24ul - (countl_zero(n.c) >> 3)};
  // #endif
}
...
struct StringHashTableGrower
    : public cider::hashtable::HashTableGrowerWithPrecalculation<initial_size_degree> {
...
struct StringHashTableLookupResult {
  ...
  explicit StringHashTableLookupResult(Mapped* mapped_ptr_) : mapped_ptr(mapped_ptr_) {}
  explicit StringHashTableLookupResult(std::nullptr_t) {}
  const cider::hashtable::VoidKey getKey() const { return {}; }
...
class StringHashTable : private boost::noncopyable {
 ...
 public:
  ...
  using ConstLookupResult =
      StringHashTableLookupResult<const typename cell_type::mapped_type>;
  ...
  static auto ALWAYS_INLINE dispatch(Self& self, KeyHolder&& key_holder, Func&& func) {
    ...
    if (sz == 0) {
      ...
      return func(self.m0, cider::hashtable::VoidKey{}, 0);
    }
    ...
    switch ((sz - 1) >> 3) {
      ...
      case 3:  // 25..32 bytes
      {
        // TODO(Deegue): support 256-bit type
      }
      default:  // >= 32 bytes
      {
        return func(self.ms, std::forward<KeyHolder>(key_holder), hash(x));
  ...
  struct EmplaceCallable {
    ...
    void ALWAYS_INLINE operator()(Map& map, KeyHolder&& key_holder, size_t hash) {
      ...
      mapped = StringHashTableLookupResult(&result->getMapped());
    }
  };
  ...
  size_t getBufferSizeInBytes() const {
    return m0.getBufferSizeInBytes() + m1.getBufferSizeInBytes() +

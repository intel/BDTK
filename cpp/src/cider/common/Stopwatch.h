...
/// It may cause some operations to fail with "Timeout exceeded: elapsed
/// 18446744073.709553 seconds". Takes previously returned value and returns it again if
/// time stepped back for some reason.
inline uint64_t clock_gettime_ns_adjusted(uint64_t prev_time,
                                          clockid_t clock_type = CLOCK_MONOTONIC) {
  uint64_t current_time = clock_gettime_ns(clock_type);
  if (likely(prev_time <= current_time))
    return current_time;

  /// Something probably went completely wrong if time stepped back for more than 1
  /// second.
...
class Stopwatch {
 public:
  /** CLOCK_MONOTONIC works relatively efficient (~15 million calls/sec) and doesn't lead
   * to syscall. Pass CLOCK_MONOTONIC_COARSE, if you need better performance with
   * acceptable cost of several milliseconds of inaccuracy.
   */
  explicit Stopwatch(clockid_t clock_type_ = CLOCK_MONOTONIC) : clock_type(clock_type_) {
    start();
  }
  ...
  uint64_t elapsedNanoseconds() const {
    return is_running ? nanoseconds() - start_ns : stop_ns - start_ns;
  }
  ...
  double elapsedSeconds() const {
    return static_cast<double>(elapsedNanoseconds()) / 1000000000ULL;
  }
...
/// Allows to atomically compare the elapsed time with a threshold and restart the watch
...
class AtomicStopwatch {
 public:
  ...
  uint64_t elapsed() const {
    uint64_t current_start_ns = start_ns;
  ...
  bool compareAndRestart(double seconds) {
    uint64_t threshold = static_cast<uint64_t>(seconds * 1000000000.0);
    uint64_t current_start_ns = start_ns;
    uint64_t current_ns = nanoseconds(current_start_ns);
  ...
  /** If specified amount of time has passed and timer is not locked right now, then
   * returns Lock object, which locks timer and, on destruction, restarts timer and
   * releases the lock. Otherwise returns object, that is implicitly casting to false.
   * This is done atomically.
   *
   * Usage:
   * if (auto lock = timer.compareAndRestartDeferred(1))
   *        /// do some work, that must be done in one thread and not more frequently than
   * each second.
   */
  Lock compareAndRestartDeferred(double seconds) {
    uint64_t threshold = uint64_t(seconds * 1000000000.0);
    uint64_t current_start_ns = start_ns;
    uint64_t current_ns = nanoseconds(current_start_ns);
    ...
    while (true) {
      ...
      if (start_ns.compare_exchange_weak(current_start_ns,
                                         current_ns | 0x8000000000000000ULL))
        return Lock(this);
    }
  }

 private:
  std::atomic<uint64_t> start_ns;
  std::atomic<bool> lock{false};
  clockid_t clock_type;

  /// Most significant bit is a lock. When it is set, compareAndRestartDeferred method
  /// will return false.
  uint64_t nanoseconds(uint64_t prev_time) const {

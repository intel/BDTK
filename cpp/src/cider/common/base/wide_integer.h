...
class Integer {
 public:
  ...
  constexpr Integer<Bits, Signed>& operator++() noexcept(
      std::is_same_v<Signed, unsigned>);
  constexpr Integer<Bits, Signed> operator++(int) noexcept(
      std::is_same_v<Signed, unsigned>);
  constexpr Integer<Bits, Signed>& operator--() noexcept(
      std::is_same_v<Signed, unsigned>);
  constexpr Integer<Bits, Signed> operator--(int) noexcept(
...
using _only_integer =
    typename std::enable_if<IntegralConcept<T>() && IntegralConcept<T2>()>::type;
...
template <typename Integral,
          typename Integral2,
          class = _only_integer<Integral, Integral2>>
...
using Int128 = cider::wide::Integer<128, signed>;
using UInt128 = cider::wide::Integer<128, unsigned>;
using Int256 = cider::wide::Integer<256, signed>;
using UInt256 = cider::wide::Integer<256, unsigned>;

...
static void decompress(const SQLTypeInfo& ti,
                       int8_t* compressed,
                       VarlenDatum* result,
                       Datum* datum) {
  switch (ti.get_type()) {
...
void ChunkIter_get_next(ChunkIter* it,
                        bool uncompress,
                        VarlenDatum* result,
                        bool* is_end) {
...
// @brief get nth element in Chunk.  Does not change ChunkIter state
void ChunkIter_get_nth(ChunkIter* it, int n, ArrayDatum* result, bool* is_end) {
  if (static_cast<size_t>(n) >= it->num_elems || n < 0) {
    *is_end = true;
    result->length = 0;
    result->pointer = NULL;
...
void ChunkIter_get_nth_varlen(ChunkIter* it, int n, ArrayDatum* result, bool* is_end) {
...
void ChunkIter_get_nth_point_coords(ChunkIter* it,
                                    int n,
                                    ArrayDatum* result,
                                    bool* is_end) {
  if (static_cast<size_t>(n) >= it->num_elems || n < 0) {

...
int32_t extract_month_fast(const int64_t lcltime) {
...
int32_t extract_quarter_fast(const int64_t lcltime) {
...
int32_t extract_year_fast(const int64_t lcltime) {
...
template <unsigned OFFSET>
ALWAYS_INLINE int64_t extract_week(const int64_t timeval) {
  int64_t const day = floor_div(timeval, kSecsPerDay);
  unsigned const doe = unsigned_mod(day - kEpochAdjustedDays, kDaysPer400Years);
  unsigned const yoe = (doe - doe / 1460 + doe / 36524 - (doe == 146096)) / 365;

...
#define DATE_TRUNC_FUNC_JIT(funcname)                                         \
  extern "C" RUNTIME_EXPORT ALWAYS_INLINE int64_t funcname(int64_t timeval) { \
...
ALWAYS_INLINE int64_t datetrunc_week(int64_t timeval) {
  // Truncate to OFFSET.
  return timeval - unsigned_mod(timeval + OFFSET * kSecsPerDay, 7 * kSecsPerDay);
}

extern "C" RUNTIME_EXPORT ALWAYS_INLINE int64_t datetrunc_week_monday(int64_t timeval) {
  return datetrunc_week<dtMONDAY>(timeval);
}

extern "C" RUNTIME_EXPORT ALWAYS_INLINE int64_t datetrunc_week_sunday(int64_t timeval) {
  return datetrunc_week<dtSUNDAY>(timeval);
}

extern "C" RUNTIME_EXPORT ALWAYS_INLINE int64_t datetrunc_week_saturday(int64_t timeval) {
...
struct EraTime {
  ...
  static EraTime make(int64_t const time) {
  ...
  EraTime operator-() const { return {-era, -yoe, -moy, -dom, -sod}; }

  EraTime operator-(EraTime const& rhs) {
  ...
  int sign(Field const field) const {
    switch (field) {
      case ERA:
        if (era != 0) {
          return era < 0 ? -1 : 1;
  ...
  int64_t count(DatetruncField const field) const {
...
extern "C" RUNTIME_EXPORT int64_t DateDiff(const DatetruncField datepart,
...
extern "C" RUNTIME_EXPORT int64_t DateDiffHighPrecision(const DatetruncField datepart,
                                                        const int64_t startdate,
                                                        const int64_t enddate,
...
extern "C" RUNTIME_EXPORT int64_t DateDiffNullable(const DatetruncField datepart,
                                                   const int64_t startdate,
                                                   const int64_t enddate,
                                                   const int64_t null_val) {

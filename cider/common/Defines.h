 ...
 * Smaller values give better cache locality, less consumption of RAM, but more overhead
 * to process the query.
 */
#define DEFAULT_BLOCK_SIZE \
  65409  /// 65536 - PADDING_FOR_SIMD - (PADDING_FOR_SIMD - 1) bytes padding that we
         /// usually have in arrays

/** Which blocks should be formed for insertion into the table, if we control the
 * formation of blocks. (Sometimes the blocks are inserted exactly such blocks that have
 * been read / transmitted from the outside, and this parameter does not affect their
 * size.) More than DEFAULT_BLOCK_SIZE, because in some tables a block of data on the disk
 * is created for each block (quite a big thing), and if the parts were small, then it
 * would be costly then to combine them.
 */
#define DEFAULT_INSERT_BLOCK_SIZE \
  1048449  /// 1048576 - PADDING_FOR_SIMD - (PADDING_FOR_SIMD - 1) bytes padding that we
           /// usually have in arrays

/** The same, but for merge operations. Less DEFAULT_BLOCK_SIZE for saving RAM (since all
 * the columns are read). Significantly less, since there are 10-way mergers.
 */
...
/// replica error max cap, this is to prevent replica from accumulating too many errors
/// and taking to long to recover.
...
/// Actually, there may be multiple acquisitions of different locks for a given table
/// within one query. Check with IStorage class for the list of possible locks
...
/// Sanitizers are using quick "frame walking" stack unwinding (this implies
/// -fno-omit-frame-pointer) And they do unwinding frequently (on every malloc/free,
/// thread/mutex operations, etc). They change %rbp during unwinding and it confuses
/// libunwind if signal comes during sanitizer unwinding
///  and query profiler decide to unwind stack with libunwind at this moment.
///
/// Symptoms: you'll get silent Segmentation Fault - without sanitizer message and without
/// usual ClickHouse diagnostics.
///

...
#error \
    "The only supported platforms are x86_64 and AArch64, PowerPC (work in progress) and RISC-V 64 (experimental)"
#endif
...
/// Explicitly allow undefined behaviour for certain functions. Use it as a function
/// attribute. It is useful in case when compiler cannot see (and exploit) it, but UBSan
/// can. Example: multiplication of signed integers with possibility of overflow when both
/// sides are from user input.
#if defined(__clang__)
#define NO_SANITIZE_UNDEFINED __attribute__((__no_sanitize__("undefined")))
#define NO_SANITIZE_ADDRESS __attribute__((__no_sanitize__("address")))
#define NO_SANITIZE_THREAD __attribute__((__no_sanitize__("thread")))
#define ALWAYS_INLINE_NO_SANITIZE_UNDEFINED \
  __attribute__((__always_inline__, __no_sanitize__("undefined")))
#else  /// It does not work in GCC. GCC 7 cannot recognize this attribute and GCC 8 simply
       /// ignores it.
...
/// Heavy assertions (that run loops or call complex functions) are allowed in debug
/// builds only. Also it makes sense to call abort() instead of __builtin_unreachable() in
/// debug builds, because SIGABRT is easier to debug than SIGTRAP (the second one makes
/// gdb crazy)
#if !defined(chassert)
#if defined(ABORT_ON_LOGICAL_ERROR)
...
/// Macros for Clang Thread Safety Analysis (TSA). They can be safely ignored by other
/// compilers. Feel free to extend, but please stay close to
/// https://clang.llvm.org/docs/ThreadSafetyAnalysis.html#mutexheader
#if defined(__clang__)
#define TSA_GUARDED_BY(...) \
  __attribute__((guarded_by(__VA_ARGS__)))  /// data is protected by given capability
#define TSA_PT_GUARDED_BY(...)  \
  __attribute__((pt_guarded_by( \
      __VA_ARGS__)))  /// pointed-to data is protected by the given capability
#define TSA_REQUIRES(...)             \
  __attribute__((requires_capability( \
      __VA_ARGS__)))  /// thread needs exclusive possession of given capability
#define TSA_REQUIRES_SHARED(...)             \
  __attribute__((requires_shared_capability( \
      __VA_ARGS__)))  /// thread needs shared possession of given capability
#define TSA_ACQUIRED_AFTER(...) \
  __attribute__(                \
      (acquired_after(__VA_ARGS__)))  /// annotated lock must be locked after given lock
#define TSA_NO_THREAD_SAFETY_ANALYSIS \
  __attribute__((no_thread_safety_analysis))  /// disable TSA for a function

/// Macros for suppressing TSA warnings for specific reads/writes (instead of suppressing
/// it for the whole function) They use a lambda function to apply function attribute to a
/// single statement. This enable us to suppress warnings locally instead of suppressing
/// them in the whole function Consider adding a comment when using these macros.
...
#define TSA_SUPPRESS_WARNING_FOR_WRITE(x) \
  ([&]() TSA_NO_THREAD_SAFETY_ANALYSIS -> auto& { return (x); }())
...
/// (Seems like there's no way to verify it, but it makes sense to distinguish it from
/// TSA_SUPPRESS_WARNING_FOR_READ for readability)
...
/// A template function for suppressing warnings about unused variables or function
/// results.

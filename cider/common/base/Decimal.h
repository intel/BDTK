...
template <class T>
auto is_decimal = std::is_same_v<T, Decimal32> || std::is_same_v<T, Decimal64> ||
                  std::is_same_v<T, Decimal128> || std::is_same_v<T, Decimal256> ||
                  std::is_same_v<T, DateTime64>;

template <class T>
auto is_over_big_int = std::is_same_v<T, Int128> || std::is_same_v<T, UInt128> ||
...
/// To perform operations, you also have to provide a scale (number of digits after
/// point).
template <typename T>
struct Decimal {
  using NativeType = T;

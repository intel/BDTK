 ...
 private:
  ...
  constexpr static integer<Bits, Signed> multiply(const integer<Bits, Signed>& lhs,
    ...
    if constexpr (Bits == 256 && sizeof(base_type) == 8) {
      ...
      CompilerUInt128 a =
          (CompilerUInt128(lhs.items[1]) << 64) +
          lhs.items[0];  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
      CompilerUInt128 b =
          (CompilerUInt128(rhs.items[1]) << 64) +
          rhs.items[0];  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
 ...
 public:
  ...
  constexpr static integer<Bits2, unsigned> divide(integer<Bits2, unsigned>& numerator,
    ...
    if constexpr (Bits == 128 && sizeof(base_type) == 8) {
      ...
      CompilerUInt128 a =
          (CompilerUInt128(numerator.items[1]) << 64) +
          numerator
              .items[0];  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)
      CompilerUInt128 b =
          (CompilerUInt128(denominator.items[1]) << 64) +
          denominator
              .items[0];  // NOLINT(clang-analyzer-core.UndefinedBinaryOperatorResult)

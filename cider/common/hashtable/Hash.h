 ...
 * Faster than intHash32 when inserting into the hash table UInt64 -> UInt64, where the
 * key is the visitor ID.
 */
...
// template <typename T>
// requires std::has_unique_object_representations_v<T> && (sizeof(T) % sizeof(DB::UInt64)
// == 0) inline DB::UInt64 intHashCRC32(const T & x, DB::UInt64 updated_value)
// {
...
inline UInt32 updateWeakHash32(const DB::UInt8* pos,
                               size_t size,
                               DB::UInt32 updated_value) {
  if (size < 8) {
    UInt64 value = 0;
...
struct UUIDHash {
  size_t operator()(DB::UUID x) const { return UInt128Hash()(x.toUnderType()); }
};

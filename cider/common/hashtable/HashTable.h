 ...
 * For example, there is an implementation of an instantly clearable hash table -
 * ClearableHashMap. For it, each cell holds the version number, and in the hash table
 * itself is the current version. When clearing, the current version simply increases; All
 * cells with a mismatching version are considered empty. Another example: for an
 * approximate calculation of the number of unique visitors, there is a hash table for
 * UniquesHashSet. It has the concept of "degree". At each overflow, cells with keys that
 * do not divide by the corresponding power of the two are deleted.
 */
 ...
 * Complex types are compared by operator== as usual (this is important if there are
 * gaps).
 *
  ...
  if constexpr (std::is_floating_point_v<RealT>)
    return 0 ==
           memcmp(
               ...
               &b,
               sizeof(
                   RealT));  /// Note that memcmp with constant size is compiler builtin.
  else
    return a == b;
}

/**
 * getKey/Mapped -- methods to get key/"mapped" values from the LookupResult returned by
 * find() and emplace() methods of HashTable. Must not be called for a null LookupResult.
 *
 * We don't use iterators for lookup result. Instead, LookupResult is a pointer of some
 * kind. There are methods getKey/Mapped, that return references or values to key/"mapped"
 * values.
 *
 ...
 *    FixedHashTable or StringHashTable. Neither GetKey nor GetMapped are supported, the
 * only valid operation is checking LookupResult for null.
 *
 * 2) Hash maps that do not store the key, e.g. FixedHashMap or StringHashMap. Only
 * GetMapped is supported.
 *
 * 3) Hash tables that store the key and do not have a "mapped" value, e.g. the normal
 * HashTable. GetKey returns the key, and GetMapped returns a zero void pointer. This
 * simplifies generic code that works with mapped values: it can overload on the return
 * type of GetMapped(), and doesn't need other parameters. One example is
 * insertSetMapped() function.
 *
 * 4) Hash tables that store both the key and the "mapped" value, e.g. HashMap. Both
 * GetKey and GetMapped are supported.
 *
...
struct HashTableCell {
  ...
  /// Whether the key is zero. In the main buffer, cells with a zero key are considered
  /// empty. If zero keys can be inserted into the table, then the cell for the zero key
  /// is stored separately, not in the main buffer. Zero keys must be such that the
  /// zeroed-down piece of memory is a zero key.
  ...
  /// Do the hash table need to store the zero key separately (that is, can a zero key be
...
void insertSetMapped(MappedType& dest, const ValueType& src) {
  dest = src.second;
}

/** Determines the size of the hash table, and when and how much it should be resized.
 * Has very small state (one UInt8) and useful for Set-s allocated in automatic memory
...
struct HashTableGrower {
  ...
  /// If collision resolution chains are contiguous, we can implement erase operation by
  /// moving the elements.
  ...
  /// Whether the hash table is sufficiently full. You need to increase the size of the
  /// hash table, or remove something unnecessary from it.
  bool overflow(size_t elems) const { return elems > maxFill(); }

  /// Increase the size of the hash table.
  void increaseSize() { size_degree += size_degree >= 23 ? 1 : 2; }

  /// Set the buffer size by the number of elements in the hash table. Used when
  /// deserializing a hash table.
 ...
 * Also, the compiler can not completely remove the code of passing through the collision
 * resolution chain, although it is not needed. NOTE: Better to use FixedHashTable
 * instead.
 */
...
struct HashTableFixedGrower {
  ...
  /// You could write UNREACHABLE(), but the compiler does not optimize everything, and it
  /// turns out less efficiently.
...
struct AllocatorBufferDeleter<true, Allocator, Cell> {
  AllocatorBufferDeleter(Allocator& allocator_, size_t size_)
      : allocator(allocator_), size(size_) {}
                                          ...
                                          Cell>  /// empty base optimization
{
 ...
 protected:
  ...
  size_t ALWAYS_INLINE findCell(const Key& x,
                                size_t hash_value,
  ...
  /// Find an empty cell, starting with the specified position and further along the
  ...
  void resize(size_t for_num_elems = 0, size_t for_buf_size = 0) {
     ...
     *  changing the variable `grower` (which determines the buffer size of the hash
     * table) is postponed for a moment after a real buffer change. The temporary variable
     * `new_grower` is used to determine the new size.
     */
     ...
     */
    using Deleter =
        AllocatorBufferDeleter<Cell::need_to_notify_cell_during_move, Allocator, Cell>;
     ...
     *  or move to the left of the collision resolution chain, because the elements to the
     * left of it have been moved to the new "right" location.
     */
    size_t i = 0;
    for (; i < old_size; ++i)
     ...
     *    if the element was to be at the end of the old buffer,                  [ x] but
     * is at the beginning because of the collision resolution chain,      [o       x]
  ...
  size_t reinsert(Cell& x, size_t hash_value) {
    ...
    /// Then the elements that previously were in collision with this can move to the old
    /// place.
    return place_value;
  }
  ...
  class iterator_base  /// NOLINT
  {
   ...
   public:
    ...
    Derived& operator++() {
      /// If iterator was pointed to ZeroValueStorage, move it to the beginning of the
      /// main buffer.
    ...
    size_t getCollisionChainLength() const {
      return container->grower.place((ptr - container->buf) -
                                     container->grower.place(getHash()));
    }
 ...
 public:
  ...
  HashTable(HashTable&& rhs) noexcept : buf(nullptr) { *this = std::move(rhs); }
 ...
 protected:
  ...
  bool ALWAYS_INLINE emplaceIfZero(const Key& x,
                                   LookupResult& it,
                                   bool& inserted,
                                   size_t hash_value) {
  ...
  void ALWAYS_INLINE emplaceNonZeroImpl(size_t place_value,
  ...
  /// Only for non-zero keys. Find the right place, insert the key there, if it does not
  /// already exist. Set iterator to the cell in output parameter.
  template <typename KeyHolder>
  void ALWAYS_INLINE emplaceNonZero(KeyHolder&& key_holder,
                                    LookupResult& it,
                                    bool& inserted,
                                    size_t hash_value) {
    const auto& key = keyHolderGetKey(key_holder);
 ...
 public:
  ...
  /// Insert a value. In the case of any more complex values, it is better to use the
  /// `emplace` function.
  ...
  template <typename KeyHolder>
  void ALWAYS_INLINE
  emplace(KeyHolder&& key_holder, LookupResult& it, bool& inserted, size_t hash_value) {
    const auto& key = keyHolderGetKey(key_holder);
    if (!emplaceIfZero(key, it, inserted, hash_value))
      emplaceNonZero(key_holder, it, inserted, hash_value);
  }

  /// Copy the cell from another hash table. It is assumed that the cell is not zero, and
  /// also that there was no such key in the table yet.
  ...
  // requires Grower::performs_linear_probing_with_single_step
  {
     ...
     * Algorithm without recomputing hash but keep probes difference value (difference of
     * natural cell position and inserted one) in cell
     * https://arxiv.org/ftp/arxiv/papers/0909/0909.2547.pdf
     *
     * Currently we use algorithm with hash recomputing on each step from
     * https://en.wikipedia.org/wiki/Open_addressing
     */
    ...
    /**
     * During element deletion there is a possibility that the search will be broken for
     * one of the following elements, because this place erased_key_position is empty. We
     * will check next_element. Consider a sequence from (erased_key_position,
     * next_element], if the optimal_position of next_element falls into it, then removing
     * erased_key_position will not break search for next_element. If optimal_position of
     * the element does not fall into the sequence (erased_key_position, next_element]
     * then deleting a erased_key_position will break search for it, so we need to move
     * next_element to erased_key_position. Now we have empty place at next_element, so we
     * apply the identical procedure for it. If an empty element is encountered then means
     * that there is no more next elements for which we can break the search so we can
     * exit.
     */

    /// Walk to the right through collision resolution chain and move elements to better
    /// positions
    while (true) {
      ...
      /// The second condition is tricky - if the chain was overlapped before
      /// erased_key_position,
      ...
      memcpy(static_cast<void*>(&buf[erased_key_position]),

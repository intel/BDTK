 ...
 *
 * Instead of this class, you could just use the pair (version, key) in the HashSet as the
 * key but then the table would accumulate all the keys that it ever stored, and it was
 * unreasonably growing. This class goes a step further and considers the keys with the
 * old version empty in the hash table.
 */
...
class ClearableHashSetWithSavedHash
    : public HashTable<Key,
                       ClearableHashTableCell<
                           Key,
                           HashSetCellWithSavedHash<Key, Hash, ClearableHashSetState>>,
                       Hash,
                       Grower,
                       Allocator> {
 public:
...
//         (1ULL << initial_size_degree) * sizeof(ClearableHashTableCell<Key,

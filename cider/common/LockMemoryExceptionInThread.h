...
struct LockMemoryExceptionInThread {
 ...
 public:
  ...
  explicit LockMemoryExceptionInThread(VariableContext level_ = VariableContext::User,
                                       bool block_fault_injections_ = true);
  ...
  static bool isBlocked(VariableContext current_level, bool fault_injection) {
    return counter > 0 && current_level >= level &&
           (!fault_injection || block_fault_injections);
  }
};

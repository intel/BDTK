...
std::unique_ptr<CodegenColValues> CodeGenerator::codegen(const Analyzer::Expr* expr,
  ...
  auto col_var = dynamic_cast<const Analyzer::ColumnVar*>(expr);
  if (col_var) {
    return codegenColumnExpr(col_var, fetch_columns, co);
  }
  auto constant = dynamic_cast<const Analyzer::Constant*>(expr);
  if (constant) {
...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenConstantExpr(
  ...
  // TODO: Refactor constant codegen procedure.
  auto constant_value = codegen(constant_expr, ti.get_compression(), 0, co);
...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenBinOper(
  ...
  if (IS_COMPARISON(optype)) {
    return codegenCmpFun(bin_oper, co);
  }
  if (IS_LOGIC(optype)) {
...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenUOper(
  ...
  switch (optype) {
    case kNOT: {
      return codegenLogicalFun(u_oper, co);
    }
    case kCAST: {
      return codegenCastFun(u_oper, co);
    }
    case kUMINUS: {
      return codegenUMinusFun(u_oper, co);
    }
    case kISNULL: {
      return codegenIsNullFun(u_oper, co);
    }
    case kUNNEST:
      // TBD: Array support
    default:
...
llvm::Value* CodeGenerator::codegen(const Analyzer::WidthBucketExpr* expr,
  ...
  // compute width_bucket's expresn range and check the possibility of avoiding oob
  // check
  auto col_range =
          ...
          return [this, hoisted_quals, co](llvm::BasicBlock* true_bb,
            ...
            for (const auto& qual : hoisted_quals) {
              if (plan_state_->hoisted_filters_.insert(qual).second) {
                // qual was inserted into the hoisted filters map, which means we have
...
void Executor::redeclareFilterFunction() {
  ...
  for (auto bb_it = cgen_state_->filter_func_->begin();
    ...
    for (auto instr_it = bb_it->begin(); instr_it != bb_it->end(); ++instr_it) {
      ...
      for (auto op_it = instr_it->value_op_begin(); op_it != instr_it->value_op_end();
        ...
        // The last LLVM operand on a call instruction is the function to be called.
        // Never remap it.
  ...
  // Create filter_func2 with parameters only for those row func values that are known
  // to be used in the filter func code.
          ...
          const auto body_loops_entry_bb = JoinLoop::codegen(
              ...
              [this,
               ...
               &ra_exe_unit](const std::vector<llvm::Value*>& prev_iters) {
                ...
                if (can_return_error || co.needs_error_check ||
...
Executor::GroupColLLVMValue Executor::groupByColumnCodegen(
  ...
  CodeGenerator code_generator(this);
  std::vector<llvm::Value*> group_key_vec =
      code_generator.codegen(group_by_col, true, co);
  llvm::Value* group_key;
  if (3 == group_key_vec.size()) {
    CHECK(groups_buffer);
    group_key = cgen_state_->emitCall(
        "cider_get_string_id", {groups_buffer, group_key_vec[1], group_key_vec[2]});
  } else {
    group_key = group_key_vec.front();
  }
...
std::unique_ptr<FixedSizeColValues> Executor::groupByColumnCodegen(
  ...
  CHECK_GE(col_width, sizeof(int32_t));
  CodeGenerator code_generator(this);
  auto group_key = code_generator.codegen(group_by_col, co, true).release();

  // TBD: Cast for group key with kArray type when the exper is UOp.
  cgen_state_->cider_group_by_expr_cache_.push_back(group_key->copy());

  if (auto fixed_size_group_key = dynamic_cast<FixedSizeColValues*>(group_key)) {
    // TBD: Remove bitcast ?
    fixed_size_group_key->setValue(cgen_state_->ir_builder_.CreateBitCast(
        cgen_state_->castToTypeIn(fixed_size_group_key->getValue(), col_width * 8),
        get_int_type(col_width * 8, cgen_state_->context_)));

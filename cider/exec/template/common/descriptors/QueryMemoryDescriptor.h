...
class QueryMemoryDescriptor {
 public:
  ...
  size_t getNullVectorOffsetOfGroupKeys() const;
  size_t getNullVectorOffsetOfGroupTargets() const;
  ...
  bool hasCountDistinct() const {
    for (int i = 0; i < getCountDistinctDescriptorsSize(); i++) {
      if (getCountDistinctDescriptor(i).impl_type_ != CountDistinctImplType::Invalid) {
        return true;
      }
    }
    return false;
  ...
  size_t getBufferSizeBytes(const size_t override_entry_count) const;
  ...
  bool useCiderDataFormat() const { return cider_data_format_; }

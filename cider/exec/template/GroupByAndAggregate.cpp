...
ColRangeInfo GroupByAndAggregate::getColRangeInfo() {
  const int64_t baseline_threshold = Executor::baseline_threshold;
  if (ra_exe_unit_.groupby_exprs.size() != 1) {
    try {
      ...
      for (const auto& groupby_expr : ra_exe_unit_.groupby_exprs) {
        ...
        if (dynamic_cast<Analyzer::CaseExpr*>(groupby_expr.get())) {
  ...
  if (dynamic_cast<Analyzer::CaseExpr*>(ra_exe_unit_.groupby_exprs.front().get())) {
    return {QueryDescriptionType::GroupByBaselineHash, 0, 0, 0, col_range_info.has_nulls};
  }
  ...
  if (groupby_expr_ti.is_string() && !col_range_info.bucket) {
    // Currently, all returned col_range_info should be baselinehash.
    return {QueryDescriptionType::GroupByBaselineHash,
            col_range_info.min,
            col_range_info.max,
            0,
...
bool useBitMapForCountDinstinct(SQLTypes type) {
  switch (type) {
    case kTINYINT:
    case kSMALLINT:
      return true;
...
CountDistinctDescriptors init_count_distinct_descriptors(
  ...
  for (const auto target_expr : ra_exe_unit.target_exprs) {
    ...
    if (is_distinct_target(agg_info)) {
      ...
      // if arg is tinyint/smallint, use bitmap
      // else we will use Hashset since bitmap will be too large
      if (arg_range_info.isEmpty() && useBitMapForCountDinstinct(arg_ti.get_type())) {
        count_distinct_descriptors.emplace_back(CountDistinctDescriptor{
            CountDistinctImplType::Bitmap,
            arg_ti.get_type(),
            get_min_value(arg_ti.get_type()),
            get_max_value(arg_ti.get_type()) - get_min_value(arg_ti.get_type()) + 1,
      ...
      if (arg_range_info.isEmpty() && !useBitMapForCountDinstinct(arg_ti.get_type())) {
        count_distinct_descriptors.emplace_back(
            CountDistinctDescriptor{CountDistinctImplType::HashSet,
                                    arg_ti.get_type(),
                                    arg_range_info.min,
                                    0,
                                    agg_info.agg_kind == kAPPROX_COUNT_DISTINCT,
      ...
      count_distinct_descriptors.emplace_back(CountDistinctDescriptor{
          CountDistinctImplType::Invalid, SQLTypes::kNULLT, 0, 0, false, 0});
    }
  }
    ...
    if (is_group_by) {
      ...
      if (query_mem_desc.usesGetGroupValueFast() ||
        ...
        can_return_error = codegenAggCalls(
            agg_out_ptr_w_idx, varlen_output_buffer, {}, query_mem_desc, co, filter_cfg);
      } else {
        {
        ...
        can_return_error = !co.use_cider_groupby_hash;
        if (query_mem_desc.getQueryDescriptionType() ==
          ...
          LL_BUILDER.CreateRet(LL_INT(int32_t(-1)));
        }
      }
    } else {
...
std::tuple<llvm::Value*, llvm::Value*> GroupByAndAggregate::codegenGroupBy(
  ...
  llvm::Value* null_vector_ptr = nullptr;
    ...
    if (query_mem_desc.getQueryDescriptionType() ==
      ...
      group_key =
          LL_BUILDER.CreateAlloca(llvm::Type::getInt64Ty(LL_CONTEXT), key_size_lv);
               ...
               QueryDescriptionType::GroupByBaselineHash) {
      if (co.use_cider_data_format) {
        size_t group_key_num = query_mem_desc.getGroupbyColCount();
        size_t null_vector_num =
            ((group_key_num + col_width_size * 8 - 1) / (col_width_size * 8));
        key_size_lv = LL_INT(static_cast<int32_t>(group_key_num + null_vector_num));
        llvm::IntegerType* type;
        if (4 == col_width_size) {
          type = llvm::Type::getInt32Ty(LL_CONTEXT);
        ...
        group_key = alloca_inst;
        null_vector_ptr = LL_BUILDER.CreateGEP(group_key, LL_INT(group_key_num));
        for (size_t i = 0; i < null_vector_num; ++i) {
          (4 == col_width_size)
              ? LL_BUILDER.CreateStore(LL_INT((int32_t)0),
                                       LL_BUILDER.CreateGEP(null_vector_ptr, LL_INT(i)))
              : LL_BUILDER.CreateStore(LL_INT((int64_t)0),
                                       LL_BUILDER.CreateGEP(null_vector_ptr, LL_INT(i)));
        }
      } else {
        key_size_lv = LL_INT(static_cast<int32_t>(query_mem_desc.getGroupbyColCount()));
        group_key =
            col_width_size == sizeof(int32_t)
                ...
                : LL_BUILDER.CreateAlloca(llvm::Type::getInt64Ty(LL_CONTEXT),
                                          key_size_lv);
      }
    }
    CHECK(group_key);
    ...
    if (co.use_cider_data_format) {
      ...
      const auto group_expr_lvs =
          executor_->groupByColumnCodegen(group_expr.get(),
                                          ...
                                          groups_buffer);
      group_expr_lv = group_expr_lvs.translated_value;
      original_value = group_expr_lvs.original_value;
    }
    if (query_mem_desc.isSingleColumnGroupByWithPerfectHash()) {
      CHECK_EQ(size_t(1), ra_exe_unit_.groupby_exprs.size());
      ...
      if (co.use_cider_data_format) {
        executor_->cgen_state_->emitCall(
            std::string("set_group_key_slot_") +
                (4 == col_width_size ? "int32" : "int64"),
            {group_key,
             LL_BUILDER.CreateBitCast(null_vector_ptr,
                                      llvm::Type::getInt8PtrTy(LL_CONTEXT)),
             LL_INT((uint64_t)subkey_idx),
             group_expr_lv,
             null_value ? null_value : llvm::ConstantInt::getFalse(LL_CONTEXT)});
      } else {
...
std::unique_ptr<CodegenColValues> GroupByAndAggregate::codegenAggArgCider(
    const Analyzer::Expr* target_expr,
    const CompilationOptions& co) {
  AUTOMATIC_IR_METADATA(executor_->cgen_state_.get());
  CodeGenerator code_generator(executor_);

  // TBD: ArrayExpr support.
  // TBD: FunctionExpr support.
  ...
  if (auto agg_expr = dynamic_cast<const Analyzer::AggExpr*>(target_expr)) {
    ...
    return code_generator.codegen(target_expr, co, false);
  }
}

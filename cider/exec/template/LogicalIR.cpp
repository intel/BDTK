...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenLogicalFun(
  ...
  // TBD: Short circuit optimization.

  switch (optype) {
    case kAND:
      return codegenAndLogicalFun(bin_oper, co);
    case kOR:
...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenAndLogicalFun(
    const Analyzer::BinOper* bin_oper,
    const CompilationOptions& co) {
  AUTOMATIC_IR_METADATA(cgen_state_);
  ...
  auto lhs_fixedsize_value = dynamic_cast<FixedSizeColValues*>(lhs_lv.get());
  auto rhs_fixedsize_value = dynamic_cast<FixedSizeColValues*>(rhs_lv.get());
  CHECK(lhs_fixedsize_value && rhs_fixedsize_value);

  auto lhs_bool = lhs->get_type_info().is_boolean()
                      ? cgen_state_->ir_builder_.CreateTrunc(
                            lhs_fixedsize_value->getValue(),
                            llvm::Type::getInt1Ty(cgen_state_->context_))
                      : toBool(lhs_fixedsize_value->getValue()),
       rhs_bool = rhs->get_type_info().is_boolean()
                      ? cgen_state_->ir_builder_.CreateTrunc(
                            rhs_fixedsize_value->getValue(),
                            llvm::Type::getInt1Ty(cgen_state_->context_))
                      : toBool(rhs_fixedsize_value->getValue());

  llvm::Value* value = cgen_state_->ir_builder_.CreateAnd(lhs_bool, rhs_bool);

  llvm::Value* null = nullptr;
  auto lhs_null = lhs_fixedsize_value->getNull(),
       rhs_null = rhs_fixedsize_value->getNull();
  if (lhs_null && rhs_null) {
    null = cgen_state_->ir_builder_.CreateAnd(lhs_null, rhs_null);
  }
  if (lhs_null) {
    auto curr_null = cgen_state_->ir_builder_.CreateAnd(lhs_null, rhs_bool);
    null = null ? cgen_state_->ir_builder_.CreateOr(null, curr_null) : curr_null;
  }
  if (rhs_null) {
    auto curr_null = cgen_state_->ir_builder_.CreateAnd(rhs_null, lhs_bool);
    null = null ? cgen_state_->ir_builder_.CreateOr(null, curr_null) : curr_null;
  }
...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenOrLogicalFun(
    const Analyzer::BinOper* bin_oper,
    const CompilationOptions& co) {
  AUTOMATIC_IR_METADATA(cgen_state_);
  ...
  llvm::Value* value = cgen_state_->ir_builder_.CreateOr(lhs_bool, rhs_bool);
  ...
  if (lhs_null) {
    auto curr_null = cgen_state_->ir_builder_.CreateAnd(
        lhs_null, cgen_state_->ir_builder_.CreateNot(rhs_bool));
    null = null ? cgen_state_->ir_builder_.CreateOr(null, curr_null) : curr_null;
  }
  if (rhs_null) {
    auto curr_null = cgen_state_->ir_builder_.CreateAnd(
        rhs_null, cgen_state_->ir_builder_.CreateNot(lhs_bool));
    null = null ? cgen_state_->ir_builder_.CreateOr(null, curr_null) : curr_null;
  }
...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenLogicalFun(
  ...
  auto fixed_size_lv = dynamic_cast<FixedSizeColValues*>(operand_lv.get());

  auto value = fixed_size_lv->getValue();
  auto is_null = fixed_size_lv->getNull();
  auto result = cgen_state_->ir_builder_.CreateNot(cgen_state_->ir_builder_.CreateTrunc(
      value, llvm::Type::getInt1Ty(cgen_state_->context_)));

  return std::make_unique<FixedSizeColValues>(result, is_null);
}
...
std::unique_ptr<CodegenColValues> CodeGenerator::codegenIsNullFun(
    const Analyzer::UOper* u_oper,
    const CompilationOptions& co) {
  AUTOMATIC_IR_METADATA(cgen_state_);
  ...
  if (ti.get_notnull()) {
    return std::make_unique<FixedSizeColValues>(
        llvm::ConstantInt::get(get_int_type(1, cgen_state_->context_), 0));
  }
  auto operand_lv = codegen(operand, co, true);
  auto nullable_lv = dynamic_cast<NullableColValues*>(operand_lv.get());
  CHECK(nullable_lv);

  return std::make_unique<FixedSizeColValues>(nullable_lv->getNull());
}
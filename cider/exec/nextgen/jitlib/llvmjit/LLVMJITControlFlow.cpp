...
void LLVMIfBuilder::build() {
  if (condition_) {
    ...
    LOG(FATAL) << "Condition builder function is missed in LLVMIfBuilder::build.";
  }
}
...
void LLVMLoopBuilder::build() {
  if (condition_) {
    ...
    auto for_update =
        llvm::BasicBlock::Create(func_.getContext(), ".Loop_Update", &func_);
    ...
    LOG(FATAL) << "Condition builder function is missed in LLVMLoopBuilder::build.";
  }
}
};  // namespace cider::jitlib
...
const int SYSTEM_PAGE_SIZE = cider::get_page_size();

int checked_open(const char* path, const bool recover) {
  auto fd = cider::open(path, O_RDWR | O_CREAT | (recover ? O_APPEND : O_TRUNC), 0644);
  if (fd > 0) {
    return fd;
  }
...
StringDictionary::StringDictionary(const DictRef& dict_ref,
  ...
  if (!isTemp_) {
    ...
    offset_file_size_ = cider::file_size(offset_fd_);
  }
  ...
  if (!isTemp_) {  // we never mmap or recover temp dictionaries
    payload_map_ =
        reinterpret_cast<char*>(cider::checked_mmap(payload_fd_, payload_file_size_));
    offset_map_ = reinterpret_cast<StringIdxEntry*>(
        cider::checked_mmap(offset_fd_, offset_file_size_));
    if (recover) {
      const size_t bytes = cider::file_size(offset_fd_);
...
StringDictionary::~StringDictionary() noexcept {
  ...
  if (payload_map_) {
    if (!isTemp_) {
      CHECK(offset_map_);
      cider::checked_munmap(payload_map_, payload_file_size_);
      cider::checked_munmap(offset_map_, offset_file_size_);
      CHECK_GE(payload_fd_, 0);
      cider::close(payload_fd_);
      CHECK_GE(offset_fd_, 0);
      cider::close(offset_fd_);
    } else {
      CHECK(offset_map_);
      free(payload_map_);
      free(offset_map_);
...
void StringDictionary::getOrAddBulk(const std::vector<String>& input_strings,
  ...
  std::shared_lock<std::shared_mutex> write_lock(rw_mutex_);
...
void StringDictionary::eachStringSerially(int64_t const generation,
  ...
  //    std::string str;  // Import buffer. Placing outside of loop should reduce
  //    allocations. size_t const n = std::min(static_cast<size_t>(generation),
  //    storageEntryCount()); CHECK_LE(n,
  //    static_cast<size_t>(std::numeric_limits<int32_t>::max()) + 1); for (unsigned id =
  //    0; id < n; ++id) {
  //      {
  //        std::shared_lock<std::shared_mutex> read_lock(rw_mutex_);
...
bool StringDictionary::checkpoint() noexcept {
  ...
  bool ret = true;
  ret =
      ret && (cider::msync((void*)offset_map_, offset_file_size_, /*async=*/false) == 0);
  ret = ret &&
        (cider::msync((void*)payload_map_, payload_file_size_, /*async=*/false) == 0);
  ret = ret && (cider::fsync(offset_fd_) == 0);
  ret = ret && (cider::fsync(payload_fd_) == 0);
  return ret;
}
...
void StringDictionary::populate_string_ids(
  ...
  for (const int32_t source_id : source_ids) {
    if (source_id == std::numeric_limits<int32_t>::min()) {
      ...
      unsigned const string_index =
          0;  // FIXME: StringDictionaryProxy::transientIdToIndex(source_id);

...
template <>
inline float inline_fp_null_array_value<float>() {
  return NULL_ARRAY_FLOAT;
}

template <>
inline double inline_fp_null_array_value<double>() {
...
CONSTEXPR inline typename serialize_detail::IntType<sizeof(T)>::type
...
template <typename T, bool array = false>
CONSTEXPR inline void set_null(T& value) {
  using TT = typename serialize_detail::IntType<sizeof(T)>::type;

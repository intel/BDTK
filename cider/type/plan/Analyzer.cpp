...
SQLTypeInfo StringOper::get_return_type(
  ...
  if (kind == SqlStringOpKind::CHAR_LENGTH) {
    /// FIXME: (YBRua) ret-type of CHAR_LENGTH should be i64 according to substrait yaml
    /// but the plan generated by Isthmus uses i32 as ret-type, and no casting is done
    /// when comparing char_length to i32 literals (e.g. LENGTH(col_2) = 10)
    /// We temporarily uses int32 here to avoid type errors,
    /// but it should be changed to int64 after Isthmus gets updated
    return SQLTypeInfo(kINT, args[0]->get_type_info().get_notnull());
  }
  if (args.empty()) {

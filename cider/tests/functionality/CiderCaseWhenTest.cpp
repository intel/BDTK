...
TEST_F(CiderCaseWhenRandomWithNullTestBase, coalesceFunctionTest) {
  assertQueryRowEqual("select coalesce(COL_INT, COL_BIGINT, 7) from test",
                      "functions/conditional/coalesce.json");
  assertQueryRowEqual("select coalesce(COL_INT, COL_BIGINT, COL_DOUBLE) from test",
...
// substrait does not supports COALESCE functions.
// and the COALESCE expression is a syntactic shortcut for the CASE expression
...
TEST_F(CiderCaseWhenRandomWithNullTestBase, coalesceWithAggTest) {
  ...
  assertQueryRowEqual("SELECT sum(COALESCE(col_double)) FROM test",
                      "SELECT sum(CASE WHEN col_double is not null THEN col_double ELSE "
                      "null END) from test");
  assertQueryRowEqual("SELECT count(COALESCE(col_double, 777)) FROM test",
                      "SELECT count(CASE WHEN col_double is not null THEN col_double "
                      "ELSE 777 END) from test");
  assertQueryRowEqual("SELECT sum(COALESCE(col_double, 777)) FROM test",
                      "SELECT sum(CASE WHEN col_double is not null THEN col_double ELSE "
                      "777 END) from test");
  assertQueryRowEqual(

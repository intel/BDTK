...
std::unique_ptr<CiderBatch> veloxVectorToCiderBatch(RowVectorPtr vec,
                                                    velox::memory::MemoryPool* pool) {
  auto [inputArrowArray, inputArrowSchema] = veloxVectorToArrow(vec, pool);

  auto allocator = std::make_shared<PoolAllocator>(pool);
  return CiderBatchUtils::createCiderBatch(allocator, inputArrowSchema, inputArrowArray);
}
...
std::shared_ptr<Analyzer::Expr> VeloxToCiderExprConverter::toCiderExpr(
  ...
  auto cider_type = ExprEvalUtils::getCiderType(vExpr->type(), false);
...
std::shared_ptr<Analyzer::Expr> VeloxToCiderExprConverter::toCiderExpr(
  ...
  auto colType = ExprEvalUtils::getCiderType(vExpr->type(), true);
...
std::shared_ptr<Analyzer::Expr> VeloxToCiderExprConverter::toCiderExpr(
  ...
  return std::make_shared<Analyzer::UOper>(
      ExprEvalUtils::getCiderType(vExpr->type(), false),
      false,
      SQLOps::kCAST,
...
std::shared_ptr<Analyzer::Expr> VeloxToCiderExprConverter::toCiderExpr(
  ...
  if (isSupportedScalarFunction(vExpr->name()) && isBinOp(vExpr)) {
    auto type = ExprEvalUtils::getCiderType(vExpr->type(), false);
    auto inputs = vExpr->inputs();
    ...
    if (leftExpr && rightExpr) {
      ...
      return std::make_shared<Analyzer::BinOper>(
          type,
          false,
          generator::getCiderSqlOps(function_name_in_cider),
          qualifier,
          leftExpr,
          rightExpr);
    }
  ...
  if (isSupportedAggFuncion(vExpr->name()) && isUOp(vExpr)) {
    ...
    if (auto agg_field =
      ...
      SQLTypeInfo agg_type = ExprEvalUtils::getCiderType(vExpr->type(), false);

...
#define SLOG(severity_or_channel)                                                 \
  if (auto _cider_logger_severity_or_channel_ = severity_or_channel;              \
      logger::fast_logging_check(_cider_logger_severity_or_channel_))             \
    if (auto _cider_logger_ = logger::Logger(_cider_logger_severity_or_channel_)) \
  _cider_logger_.stream(__FILE__, __LINE__)
...
#else
// evaluate expression in CHECK but always return false
class NullChecker {
 public:
  explicit NullChecker(bool cond) : cond_(cond) {}

  operator bool() const { return false; }

  template <typename T>
  NullChecker& operator<<(const T&) {
    return *this;
  }
...
#define CHECK(condition)                               \
  if (auto _null = logger::NullChecker(!!(condition))) \
  _null << ""

#define CHECK_OP(OP, x, y)                                       \
  if (auto _null = logger::NullChecker(logger::Check##OP(x, y))) \
  _null << ""
#endif
...
#define CIDER_CHECKOP_FUNCTION(name, op)                            \
  template <typename X, typename Y>                                 \
...
#define CIDER_CHECKOP_FUNCTION(name, op)     \
  template <typename X, typename Y>          \
  bool Check##name(X const& x, Y const& y) { \
    return true;                             \
  }
#endif
CIDER_CHECKOP_FUNCTION(EQ, ==)
CIDER_CHECKOP_FUNCTION(NE, !=)
CIDER_CHECKOP_FUNCTION(LT, <)
CIDER_CHECKOP_FUNCTION(LE, <=)
CIDER_CHECKOP_FUNCTION(GT, >)
CIDER_CHECKOP_FUNCTION(GE, >=)
#undef CIDER_CHECKOP_FUNCTION

#define UNREACHABLE() LOG(FATAL) << "UNREACHABLE "
#define UNIMPLEMENTED() LOG(FATAL) << "UNIMPLEMENTED! "
#define TODO(author, msg) LOG(WARNING) << "TODO(" #author "):" #msg  // TODO(author): msg
#else                                                                // NO_BOOST

...
struct KeyAccessor {
  KeyAccessor(const int8_t* key_buff, const size_t key_stride, const size_t key_idx)
      : buffer(key_buff), stride(key_stride), index(key_idx) {}
  ALWAYS_INLINE KeyT get(const IndexT rowid) const {
...
struct KeyComparator {
  KeyComparator(const HeapOrdering hp_order,
                const bool nullable,
                const KeyT null_val,
  ...
  ALWAYS_INLINE bool operator()(const KeyT lhs, const KeyT rhs) const {
    if (has_nulls) {
...
ALWAYS_INLINE void sift_down(NodeT* heap,
...
template <typename KeyT = int64_t, typename NodeT = int64_t>
ALWAYS_INLINE void sift_up(NodeT* heap,
                           const NodeT curr_idx,
                           const KeyComparator<KeyT>& compare,
                           const KeyAccessor<KeyT, NodeT>& accessor) {
...
template <typename KeyT = int64_t, typename NodeT = int64_t>
ALWAYS_INLINE void push_heap(int64_t* heap_ptr,
                             int64_t* rows_ptr,
                             NodeT& node_count,
...
template <typename KeyT = int64_t, typename NodeT = int64_t>
ALWAYS_INLINE bool pop_and_push_heap(int64_t* heap_ptr,
                                     int64_t* rows_ptr,
...
template <typename KeyT = int64_t>
ALWAYS_INLINE int64_t* get_bin_from_k_heap_impl(int64_t* heaps,
                                                const uint32_t k,
                                                const uint32_t row_size_quad,
                                                const uint32_t key_offset,
                                                const bool min_heap,
...
#define DEF_GET_BIN_FROM_K_HEAP(key_type)                                         \
  extern "C" RUNTIME_EXPORT NEVER_INLINE int64_t* get_bin_from_k_heap_##key_type( \
      int64_t* heaps,                                                             \
      const uint32_t k,                                                           \
      const uint32_t row_size_quad,                                               \
      const uint32_t key_offset,                                                  \

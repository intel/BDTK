...
#ifndef CIDER_FUNCTION_EXTRACTFROMTIME_H
#define CIDER_FUNCTION_EXTRACTFROMTIME_H
...
enum ExtractField {
  ...
  kMINUTE = 0x00000020,
  kSECOND = 0x00000040,
  kMILLISECOND = 0x00000080,
  kMICROSECOND = 0x00000100,
  kNANOSECOND = 0x00000200,
  kDOW = 0x00000400,
  kISODOW = 0x00000800,
  kDOY = 0x00001000,
  kEPOCH = 0x00002000,
  kQUARTERDAY = 0x00004000,
  kWEEK = 0x00008000,
  kWEEK_SUNDAY = 0x00010000,
  kWEEK_SATURDAY = 0x00020000,
  kDATEEPOCH = 0x00040000,
  kNONE = 0x00080000,
};

int64_t ExtractFromTime(ExtractField field, const int64_t timeval);

// Return floor(dividend / divisor).
// Assumes 0 < divisor.
inline int64_t floor_div(int64_t const dividend, int64_t const divisor) {
  return (dividend < 0 ? dividend - (divisor - 1) : dividend) / divisor;
}

// Return remainer r of dividend / divisor, where 0 <= r < divisor.
// Assumes 0 < divisor.
inline int64_t unsigned_mod(int64_t const dividend, int64_t const divisor) {
...
#endif  // CIDER_FUNCTION_EXTRACTFROMTIME_H

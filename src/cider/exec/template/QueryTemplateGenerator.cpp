...
std::tuple<llvm::Function*, llvm::CallInst*> query_template_impl(
  ...
  auto func_row_process = row_process<Attributes>(
      mod, is_estimate_query ? 1 : aggr_col_count, hoist_literals, use_cider_data_format);
  ...
  if (!is_estimate_query) {
    ...
    if (use_cider_data_format) {
      for (size_t i = 0; i < aggr_col_count; ++i) {
        auto result_ptr = new AllocaInst(i64_type, 0, "result.null", bb_entry);
        result_ptr->setAlignment(LLVM_ALIGN(8));
        result_ptr_vec.push_back(result_ptr);
      }
    }
  ...
  if (!is_estimate_query) {
    ...
    if (use_cider_data_format) {
      for (size_t i = 0; i < aggr_col_count; ++i) {
        auto idx_lv = ConstantInt::get(i32_type, i + aggr_col_count);
        auto agg_init_gep =
            GetElementPtrInst::CreateInBounds(agg_init_val, idx_lv, "", bb_entry);
        auto agg_init_val = new LoadInst(
            get_pointer_element_type(agg_init_gep), agg_init_gep, "", false, bb_entry);
        agg_init_val->setAlignment(LLVM_ALIGN(8));
        agg_init_val_vec.push_back(agg_init_val);
        auto init_val_st = new StoreInst(
            agg_init_val, result_ptr_vec[i + aggr_col_count], false, bb_entry);
        init_val_st->setAlignment(LLVM_ALIGN(8));
      }
    }
  }
  ...
  if (!is_estimate_query) {
    ...
    if (use_cider_data_format) {
      for (size_t i = 0; i < aggr_col_count; ++i) {
        auto result =
            new LoadInst(get_pointer_element_type(result_ptr_vec[i + aggr_col_count]),
                         result_ptr_vec[i + aggr_col_count],
                         ".pre.result.null",
                         false,
                         bb_crit_edge);
        result->setAlignment(LLVM_ALIGN(8));
  ...
  // Block  .exit
  /**
   * for each aggregate target, threads
  ...
  if (!is_estimate_query) {
    ...
    if (use_cider_data_format) {
      for (int64_t i = aggr_col_count - 1; i >= 0; --i) {
        ...
        result->addIncoming(result_vec_pre[i + aggr_col_count], bb_crit_edge);
        result->addIncoming(agg_init_val_vec[i + aggr_col_count], bb_entry);
    ...
    for (size_t i = 0; i < aggr_col_count; ++i) {
      ...
      if (use_cider_data_format) {
        auto col_idx = ConstantInt::get(i32_type, i + aggr_col_count);
        auto out_gep = GetElementPtrInst::CreateInBounds(out, col_idx, "", bb_exit);
        auto col_buffer =
            new LoadInst(get_pointer_element_type(out_gep), out_gep, "", false, bb_exit);
        col_buffer->setAlignment(LLVM_ALIGN(8));
        auto slot_idx = ConstantInt::get(i32_type, 0);
        auto target_addr =
            GetElementPtrInst::CreateInBounds(col_buffer, slot_idx, "", bb_exit);
        StoreInst* result_st =
            new StoreInst(result_vec[i + aggr_col_count], target_addr, false, bb_exit);
        result_st->setAlignment(LLVM_ALIGN(8));
      }
    }
  }
...
std::tuple<llvm::Function*, llvm::CallInst*> query_group_by_template_impl(
  ...
  CHECK(func_row_process);
  auto func_init_shared_mem = mod->getFunction("init_shared_mem_nop");
  ...
  Attributes query_func_pal;
  {
    ...
    if (enable_row_skip_mask) {
      ...
      AttrBuilder B;
      B.addAttribute(Attribute::NoAlias);
      PAS = Attributes::get(mod->getContext(), 12U, B);
  ...
  Value* error_code = &*(++query_arg_it);
  error_code->setName("error_code");
  Value* row_skip_mask = nullptr;
  if (enable_row_skip_mask) {
    row_skip_mask = &*(++query_arg_it);
    row_skip_mask->setName("row_skip_mask");
  }
  ...
  llvm::ConstantInt* shared_mem_bytes_lv = ConstantInt::get(i32_type, 0);
  ...
  if (enable_row_skip_mask) {
    auto bb_row_func = SplitBlock(bb_forbody, row_process);
    bb_row_func->setName(".row_func");

    auto bb_pos_inc = SplitBlock(bb_row_func, row_process->getNextNonDebugInstruction());
    bb_pos_inc->setName(".pos_inc");

    auto bb_row_mask_set = BasicBlock::Create(
        mod->getContext(), ".row_mask_set", query_func_ptr, bb_crit_edge);
    IRBuilder<> bb_row_mask_set_builder(bb_row_mask_set);
    auto set_bit_func = mod->getFunction("set_bit_vector");
    CHECK(set_bit_func);
    bb_row_mask_set_builder.CreateCall(set_bit_func, {row_skip_mask, pos});
    bb_row_mask_set_builder.CreateBr(bb_pos_inc);

    auto need_retry = new ICmpInst(&bb_row_func->back(),
                                   ICmpInst::ICMP_NE,
                                   &bb_row_func->front(),
                                   ConstantInt::get(i32_type, 0),
                                   "need_retry");
    ReplaceInstWithInst(&bb_row_func->back(),
                        BranchInst::Create(bb_pos_inc, bb_row_mask_set, need_retry));

    auto check_bit_vector_set = mod->getFunction("check_bit_vector_set");
    auto check_bit_set = CallInst::Create(
        check_bit_vector_set, {row_skip_mask, pos}, "", &bb_forbody->back());
    ReplaceInstWithInst(&bb_forbody->back(),
                        BranchInst::Create(bb_pos_inc, bb_row_func, check_bit_set));
  }
...
std::tuple<llvm::Function*, llvm::CallInst*> query_template(
  ...
  return query_template_impl<llvm::AttributeList>(
      module, aggr_col_count, hoist_literals, is_estimate_query, use_cider_data_format);
}
std::tuple<llvm::Function*, llvm::CallInst*> query_group_by_template(
  ...
  return query_group_by_template_impl<llvm::AttributeList>(
      module, hoist_literals, query_mem_desc, check_scan_limit, enable_row_skip_mask);
}
